import { Component, OnInit } from '@angular/core';
import { Subject, takeUntil } from 'rxjs';
import { Product } from '../../../core/models/product.model';
import { ProductsService } from '../../../shared/services/products.service';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';

@Component({
  selector: 'app-index',
  templateUrl: './index.component.html',
  styleUrl: './index.component.scss'
})
export class IndexComponent implements OnInit{

  _unsubscribeAll = new Subject<any>();

  products: Product[] = [];

  productsByGroup: any;

  pageSize = 10;
  constructor(
    private _productsService: ProductsService,
    private _modalService: NgbModal
  ){ }

  ngOnInit(): void {
    this.getProducts();
    
  }

  getProducts(){
    this._productsService.getAllProducts(this.pageSize).pipe(
      takeUntil(this._unsubscribeAll)
    ).subscribe({
      next: (res: any) => {
        this.products = res;

        // const category = this.products.reduce((acc: any, curr) => {
        //   let key = curr.category;
        //   if (!acc[key!]) {
        //     acc[key!] = [];
        //   }
        //   acc[key!].push(curr);
        //   return acc;
        // }, {});
    
        //Get the groups and tasks related.
        // this.productsByGroup = Object.keys(category).map(key => ({
        //   category: key,
        //   products: category[key]
        // }));
      },
      error: (err) => {
        // this._helperService.handleError(err);
      }
    })
  }


  selectedProdId: any;
  openModal(modal:any,selectedProdId: any) {
    this._modalService.open(modal, {
      centered: true,
      size: "lg",
      windowClass: "modal modal-dialog-centered",
      scrollable: true,
    });
    this.selectedProdId = selectedProdId
  }


  confirmDelete(){
    this._productsService.deleteProduct(this.selectedProdId)
      .pipe(
        takeUntil(this._unsubscribeAll)
      ).subscribe({
        next: (res: any) => {
          // this._helperService.handleSuccess(res);
          this.getProducts();
          this._modalService.dismissAll();
        },
        error: (err) => {
          // this._helperService.handleError(err);
        }
      })
  }

  ngOnDestroy(): void {
    this._unsubscribeAll.next(true);
  }
}

